...
Public workflows should use skill-mops-fetch so every official
...
def _find_skill_runner(repo: Path) -> Path:
    candidates = [
        repo.parent / "skills" / "common" / "skill-mops-fetch" / "scripts" / "run_mops_fetch.py",
        repo / "skills" / "common" / "skill-mops-fetch" / "scripts" / "run_mops_fetch.py",
        repo / "skills" / "skill-mops-fetch" / "scripts" / "run_mops_fetch.py",
    ]
    for candidate in candidates:
        if candidate.is_file():
            return candidate
    raise SystemExit("Cannot find skill-mops-fetch runner. Expected ../skills/common/skill-mops-fetch/ or skills/common/...")


def main() -> int:
    parser = argparse.ArgumentParser(description="Deprecated wrapper; use skill-mops-fetch.")
    ...
    print("DEPRECATED: scripts/mops_downloader.py is retired. Delegating to skill-mops-fetch.", file=sys.stderr)
