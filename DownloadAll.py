#!/usr/bin/env python3
"""
DownloadAll.py - batch orchestrator using skill-mops-fetch
...
def find_skill_runner() -> str:
    """Locate skill-mops-fetch runner from the MOPS repo."""
    repo = os.path.dirname(os.path.abspath(__file__))
    candidates = [
        os.path.join(os.path.dirname(repo), 'skills', 'common', 'skill-mops-fetch', 'scripts', 'run_mops_fetch.py'),
        os.path.join(repo, 'skills', 'common', 'skill-mops-fetch', 'scripts', 'run_mops_fetch.py'),
        os.path.join(repo, 'skills', 'skill-mops-fetch', 'scripts', 'run_mops_fetch.py'),
    ]
    for candidate in candidates:
        if os.path.isfile(candidate):
            return candidate
    raise FileNotFoundError('Cannot find skill-mops-fetch runner. Expected ../skills/common/skill-mops-fetch/.')
...
def download_company_pdf_simple(company_id: int, company_name: str, year: int, 
    ...
    """
    Download and convert reports for a single company through skill-mops-fetch.
    """
...
def main():
    parser = argparse.ArgumentParser(
        description='Download MOPS PDFs and Markdown sidecars for all companies via skill-mops-fetch',
  ...
  python DownloadAll.py --year 2025 --quarter 1 --dry-run
    ...
    if args.dry_run:
        ...
        for company_id, company_name in companies:
            ...
            cmd_str = f"python ../skills/common/skill-mops-fetch/scripts/run_mops_fetch.py {company_id} {args.year} {quarter}"
            if args.only_missing_files:
